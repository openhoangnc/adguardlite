...
//! refusal, not TLS. The TLS listeners are exercised in `sift-dns`.
...
fn state(insecure: bool) -> Shared {
    ...
    config.users = vec![sift_config::model::WebUser {
        ...
        password: sift_api::auth::hash_password("unused by these tests").expect("hashing"),
    }];
    ...
    let base = std::env::temp_dir().join(format!("sift-doh-{}", std::process::id()));
...
async fn serve(insecure: bool) -> SocketAddr {
    ...
    let app = sift_api::routes::router(state(insecure), false);
