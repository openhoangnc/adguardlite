...
pub async fn querylog_config_update(
...
) -> ApiResult<()> {
    {
        ...
        if let Some(v) = req.interval {
            cfg.querylog.interval = sift_core::GoDuration::from_millis(v);
        }
...
pub async fn querylog_config_legacy(
...
) -> ApiResult<()> {
    {
        ...
        cfg.querylog.interval = sift_core::GoDuration::from_days(req.interval as i64);
...
fn apply_querylog_config(s: &Shared) {
    let cfg = s.config.read();
    s.querylog.set_config(sift_querylog::log::Config {
        enabled: cfg.querylog.enabled,
...
/// `GET /control/stats`
pub async fn stats(State(s): State<Shared>) -> Json<sift_stats::stats::StatsResp> {
    Json(s.stats.data())
}
...
pub async fn stats_config_update(
...
) -> ApiResult<()> {
    {
        ...
        if let Some(v) = req.interval {
            cfg.statistics.interval = sift_core::GoDuration::from_millis(v);
        }
        if let Some(v) = req.ignored {
...
pub async fn stats_config_legacy(
    ...
    {
        let mut cfg = s.config.write();
        cfg.statistics.interval = sift_core::GoDuration::from_days(req.interval as i64);
...
fn apply_stats_config(s: &Shared) {
    let cfg = s.config.read();
    s.stats.set_config(sift_stats::stats::Config {
        enabled: cfg.statistics.enabled,
...
mod tests {
    ...
    use sift_querylog::entry::Result as EntryResult;
    ...
    fn the_api_entry_carries_the_winning_rule() {
        ...
        e.result.rules = vec![sift_querylog::entry::ResultRule {
            text: "||a.com^".into(),
            ip: None,
            filter_list_id: 7,
        }];
