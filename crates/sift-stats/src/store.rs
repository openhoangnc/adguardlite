...
pub fn load(path: &Path) -> Result<Vec<(u32, UnitDb)>, Error> {
    ...
    for (name, keys) in buckets {
        ...
        let u = sift_gob::decode_unit(blob).map_err(|source| Error::Gob { hour, source })?;
...
pub fn save<U: Borrow<UnitDb>>(path: &Path, units: &[(u32, U)]) -> Result<(), Error> {
    ...
    for (hour, u) in units {
        let blob = sift_gob::encode_unit(&to_gob(u.borrow()));
        buckets.insert(name_from_hour(*hour), BTreeMap::from([(vec![0u8], blob)]));
    }

    sift_bolt::write_file(path, &buckets, PAGE_SIZE)?;
...
fn from_gob(u: &sift_gob::UnitDb) -> UnitDb {
    let pairs = |v: &[sift_gob::CountPair]| -> Vec<CountPair> {
        v.iter()
            .map(|p| CountPair {
                name: p.name.clone(),
                count: p.count,
...
fn to_gob(u: &UnitDb) -> sift_gob::UnitDb {
    let pairs = |v: &[CountPair]| -> Vec<sift_gob::CountPair> {
        v.iter()
            .map(|p| sift_gob::CountPair {
                name: p.name.clone(),
                count: p.count,
            })
            .collect()
...
mod tests {
    ...
    fn tmpdir(tag: &str) -> std::path::PathBuf {
        let d = std::env::temp_dir().join(format!("sift-stats-{tag}-{}", std::process::id()));
