    ...
    pub fn choose(target: &str, rotation: Rotation) -> Self {
        ...
        if target == SYSLOG {
            return match Syslog::connect() {
                ...
                Err(e) => {
                    eprintln!("sift: connecting to the system log: {e}; using stderr");
        ...
        #[cfg(not(unix))]
        if target == SYSLOG {
            eprintln!("sift: the system log is not available on this platform");
        ...
        match RotatingFile::open(target, rotation) {
            ...
            Err(e) => {
                eprintln!("sift: opening the log file {target}: {e}; using stderr");
...
mod tests {
    ...
    fn tmpdir(tag: &str) -> PathBuf {
        let d = std::env::temp_dir().join(format!("sift-log-{tag}-{}", std::process::id()));
