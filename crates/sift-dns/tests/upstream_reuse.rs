...
fn answer(wire: &[u8], seen: &Seen) -> Vec<u8> {
    ...
    *seen.wire_id.lock() = Some(req.metadata.id);
    ...
    sift_dns::msg::with_addrs(&req, &["1.2.3.4".parse().expect("static addr")], 60)
        .to_bytes()
        .expect("encoding")
}
...
fn certificates() -> (sift_dns::tls::Loaded, Arc<rustls::ClientConfig>) {
    let mut params =
    ...
    let server = sift_dns::tls::load(&sift_dns::tls::Source {
...
async fn client(spec: &str, tls: &Arc<rustls::ClientConfig>) -> Client {
    let up = sift_dns::addr::parse(spec)
        .expect("parsing")
        .upstream
        .expect("an upstream");
...
async fn doq_server(
    ...
    let endpoint =
        sift_dns::doq::endpoint("127.0.0.1:0".parse().expect("static addr"), tls).expect("binding");
