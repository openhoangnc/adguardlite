...
use agl_dns::pool::{Mode, Pool, SharedPool};
use agl_dns::ratelimit::{Config as RlConfig, Limiter};
use agl_dns::resolver::{Action, ClientInfo, Proto, Resolver, Settings};
use agl_dns::rewrite::Table;
use agl_dns::server::{Event, Observer, Server};
...
async fn resolver(server: SocketAddr, refreshes: bool) -> Arc<Resolver> {
    let up = agl_dns::addr::parse(&server.to_string())
        .expect("parsing")
        .upstream
        .expect("an upstream");
    ...
    if refreshes {
        let (tx, rx) = agl_dns::refresh::channel();
        r.set_refresh_sender(tx);
        tokio::spawn(agl_dns::refresh::run(r.clone(), rx));
    }
...
fn answer(out: &agl_dns::resolver::Outcome) -> (String, u32) {
