...
//! up to [`agl_core::SCHEMA_VERSION`], exactly as upstream's migrator does —
...
mod tests {
    ...
    fn migrate(s: &str) -> Value {
        let mut doc = parse(s);
        upgrade(&mut doc, agl_core::SCHEMA_VERSION, &Context::default()).expect("must migrate");
    ...
    fn the_version_field_ends_at_the_target() {
        ...
        assert_eq!(schema_version(&got), agl_core::SCHEMA_VERSION);
    }
}
